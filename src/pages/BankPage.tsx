...
export function BankPage() {
  ...
  const historyPageTransactions = historyPageData?.transactions ?? []
  const historyPaginationPages = getPaginationPages(
    historyPage,
    historyTotalPages
  )

  useEffect(() => {
    if (!historyAccountId || !historySummary) {
      return
    }
            ...
            <header className="bank-history-modal-header">
              <div>
                <h2 id="bank-create-title">
                  {editingTransactionId ? 'Edit Transfer' : 'Tambah Transfer'}
                </h2>
                <p>
                  {editingTransactionId
            ...
            {historyTotal > 0 ? (
              <footer className="bank-history-pagination">
                ...
                <div className="bank-history-pagination-buttons">
                  ...
                  {historyPaginationPages.map((pageItem, index) =>
                    pageItem === 'ellipsis' ? (
                      <button
                        key={`ellipsis-${index}`}
                        type="button"
                      ...
                      <button
                        ...
                        onClick={() => changeHistoryPage(pageItem)}
